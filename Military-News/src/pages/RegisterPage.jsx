...
const RegisterPage = () => {
    ...
    const formik = useFormik({
        initialValues: { name: '', email: '', password: '', confirmPassword: '', role: 'general' },
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-military-950 px-4 relative overflow-hidden">
            ...
            <div className="w-full max-w-md bg-military-900 p-8 rounded-sm shadow-2xl border-2 border-military-600 relative z-10">
                ...
                <h2 className="text-3xl font-stencil text-center text-military-100 mb-2 tracking-widest uppercase">Enlistment Form</h2>
                <p className="text-center text-military-500 mb-8 font-mono text-xs uppercase tracking-widest">Join the Elite Forces</p>

                <form onSubmit={formik.handleSubmit} className="space-y-4">
                    <div>
                        <label className="text-xs font-mono text-military-400 uppercase ml-1">Operative Name</label>
                        <input
                            name="name"
                            type="text"
                        ...
                        />
                        {formik.touched.name && formik.errors.name && <p className="text-red-500 text-xs mt-1 ml-1 font-mono uppercase">{formik.errors.name}</p>}
                    </div>

                    <div>
                        <label className="text-xs font-mono text-military-400 uppercase ml-1">Contact (Email)</label>
                        <input
                            name="email"
                            type="email"
                            className="w-full p-3 bg-military-950 border border-military-700 text-military-100 focus:border-military-400 focus:outline-none font-mono"
                    ...
                    <div>
                        <label className="text-xs font-mono text-military-400 uppercase ml-1">Role</label>
                        <select
                            name="role"
                        ...
                        {formik.touched.role && formik.errors.role && <p className="text-red-500 text-xs mt-1 ml-1 font-mono uppercase">{formik.errors.role}</p>}
                    </div>

                    <div>
                        <label className="text-xs font-mono text-military-400 uppercase ml-1">Set Access Code</label>
                        <input
                            name="password"
                            type="password"
                            className="w-full p-3 bg-military-950 border border-military-700 text-military-100 focus:border-military-400 focus:outline-none font-mono"
                    ...
                    <div>
                        <label className="text-xs font-mono text-military-400 uppercase ml-1">Confirm Code</label>
                        <input
                        ...
                        />
                        {formik.touched.confirmPassword && formik.errors.confirmPassword && <p className="text-red-500 text-xs mt-1 ml-1 font-mono uppercase">{formik.errors.confirmPassword}</p>}
                    </div>
                ...
                <p className="text-center mt-6 text-military-500 text-xs font-mono">
                    ALREADY ENLISTED? <Link to="/login" className="text-military-300 font-bold hover:text-military-100 uppercase underline decoration-military-500">AUTHENTICATE</Link>
                </p>
            </div>
        </div>
    );
